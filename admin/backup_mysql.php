...
// the following lines check to be sure that they've been entered correctly in the language file
  $toolSearchPath = (isset($_GET['tool']) && $_GET['tool'] != '') ? $_GET['tool']:'';
  $pathsearch=array(str_replace('mysqldump.exe','',$toolSearchPath).'\\',str_replace('mysql','',LOCAL_EXE_MYSQL).'/',str_replace('mysql.exe','',LOCAL_EXE_MYSQL).'/','/usr/bin/','/usr/local/bin/','/usr/local/mysql/bin/','c:/mysql/bin/','d:/mysql/bin/','e:/mysql/bin/', 'c:/apache2triad/mysql/bin/', 'd:/apache2triad/mysql/bin/', 'e:/apache2triad/mysql/bin/', 'c:/server/mysql/bin/', '\'c:/Program Files/MySQL/MySQL Server 5.0/bin/\'', '\'d:\\Program Files\\MySQL\\MySQL Server 5.0\\bin\\\'');
...
//  	$path = str_replace('\\','/',$path); // convert backslashes
  ...
  if ($mysqldump_exe=='unknown'){
  	$messageStack->add_session('警告: 没找到"mysqldump"库。不能执行数据库备份。<br />请在langauges/backup_mysql.php中设置MYSQLDUMP库的路径','error');
    $mysqldump_exe = ((@file_exists($mysqldump_exe) ? $mysqldump_exe : 'mysqldump' ) );
  }
  if (!$mysql_exe=='unknown'){
  	$messageStack->add_session('警告: 没找"mysql"库。不能执行数据库恢复。<br />请在langauges/backup_mysql.php中设置MYSQL库的路径','error');
...
//      $resultcodes = @exec($toolfilename . $dump_params , $output, $dump_results );
        ...
        if ($debug=='ON' || (zen_not_null($dump_results) && $dump_results!='0')) $messageStack->add_session('返回代码: '.$dump_results, 'caution');
...
<?php
  ...
  switch ($action) {
    case 'backup':
      ...
      $contents = array('form' => zen_draw_form('backup', FILENAME_BACKUP_MYSQL, 'action=backupnow'.(($debug=='ON')?'&debug=ON':'').(isset($_GET['tool'])?'&tool='.$_GET['tool']:'')));
