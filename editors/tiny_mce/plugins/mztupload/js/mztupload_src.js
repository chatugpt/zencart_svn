...
function mztupload_goto_do(file){
    ...
    if(file_type == 'jpg'|| file_type == 'jpeg'|| file_type == 'png' || file_type == 'gif'){
        insert_html="<img src='"+file+"' alt='"+file_name+"' />";
        tinyMCE.execCommand('mceInsertContent', false, insert_html);
        tinyMCEPopup.close();
    }
    else if(str_else_type.indexOf(file_type)!=-1){
        insert_html="<a href='"+file+"' target='_blank'><img src='"+webpath+"/imgs/mzt_file_"+file_type+".gif' alt='"+file_name+"' /></a>";
        tinyMCE.execCommand('mceInsertContent', false, insert_html);
        tinyMCEPopup.close();
    }
    else{
        insert_html="<a href='"+file+"' target='_blank'><img src='"+webpath+"/imgs/mzt_file.gif' alt='"+file_name+"' /></a>";
        tinyMCE.execCommand('mceInsertContent', false, insert_html);
        tinyMCEPopup.close();
    }
}
