...
// class methods
   ...
   // $dest_country = $order->delivery['country']['iso_code_2'];
    ...
    $zone_query = "select zone_code
                   from " . TABLE_ZONES . "
                   where  zone_id = '" . (int)$dest_country. "'";

    $zone = $db->Execute($zone_query);

    if ($zone->RecordCount()) {
      $dest_country=$zone->fields['zone_code'];
    }
...
// shipping adjustment
    ...
    function install() {
      global $db;
	  $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('多地区运费名字修改', 'MODULE_SHIPPING_ZONES_TEXT_TITLE', 'zones', '多地区运费', '6', '0', now())");
      ...
      for ($i = 1; $i <= $this->num_zones; $i++) {
        ...
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('手续费基于订单还是箱数，地区" . $i . " (when by weight)' , 'MODULE_SHIPPING_ZONES_HANDLING_METHOD_" . $i."', 'Order', 'Do you want to charge Handling Fee Per Order or Per Box?', '6', '0', 'zen_cfg_select_option(array(\'Order\', \'Box\'), ', now())");
    ...
    function keys() {
      $keys = array('MODULE_SHIPPING_ZONES_TEXT_TITLE','MODULE_SHIPPING_ZONES_STATUS', 'MODULE_SHIPPING_ZONES_METHOD', 'MODULE_SHIPPING_ZONES_TAX_CLASS', 'MODULE_SHIPPING_ZONES_TAX_BASIS', 'MODULE_SHIPPING_ZONES_SORT_ORDER', 'MODULE_SHIPPING_ZONES_SKIPPED');
